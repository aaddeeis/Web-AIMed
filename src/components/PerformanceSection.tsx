...
export default function PerformanceSection({ lang }: PerformanceSectionProps) {
  ...
  const pastEventsMapped = (events || [])
    .filter(evt => evt.type === 'past')
    .map(evt => ({
      title: {
        en: evt.title.en,
        id: evt.title.id
      },
      category: 'Past Event / Kegiatan Terlaksana',
      date: evt.date,
      coverage: {
        en: `${evt.description.en} held at ${evt.location.en}.`,
        id: `${evt.description.id} diselenggarakan di ${evt.location.id}.`
      },
      image: evt.image || 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&q=80&w=800',
      url: '#'
    }));
                      ...
                      <div key={idx} className="glass-card p-6 sm:p-8 rounded-2xl shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
                        ...
                        {isExpanded && (
                          <div className="mt-5 pt-5 border-t border-slate-100 dark:border-slate-900 space-y-4 animate-in fade-in duration-200">
                            ...
                            <div className="p-3.5 bg-black/5 dark:bg-white/[0.02] border border-black/5 dark:border-white/5 rounded-xl text-xs text-slate-500 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                              ...
                              {item.url && item.url !== '#' ? (
                                <a 
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="text-teal-600 dark:text-teal-400 hover:text-teal-700 dark:hover:text-teal-300 flex items-center text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer"
                                >
                                  <span>{lang === 'en' ? 'Official Event Page' : 'Halaman Resmi Acara'}</span>
                                  <ExternalLink className="w-3.5 h-3.5 ml-1" />
                                </a>
                              ) : (
                                <span className="text-slate-400 dark:text-slate-500 flex items-center text-[10px] font-bold uppercase tracking-wider">
                                  <span>{lang === 'en' ? 'Official Event Page' : 'Halaman Resmi Acara'}</span>
                            ...
                            {item.url && item.url !== '#' && (
                              <div className="pt-2 flex justify-end">
                                <a 
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="text-xs font-bold text-teal-600 dark:text-teal-400 hover:text-teal-700 dark:hover:text-teal-300 flex items-center space-x-1 transition-colors cursor-pointer"
                                >
                                  <span>{lang === 'en' ? 'Visit Journal Website' : 'Kunjungi Situs Web Jurnal'}</span>
                                  <ExternalLink className="w-3.5 h-3.5" />
                                </a>
                              </div>
                            )}
