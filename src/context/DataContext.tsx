...
interface DataContextType {
  ...
  saveToServer: (force?: boolean) => Promise<{ 
    success: boolean; 
    conflict?: boolean;
    error?: string; 
    githubSync?: { enabled: boolean; success?: boolean; message?: string; error?: string } 
  }>;
}
...
const defaultLeadership: Person[] = [
  {
    ...
    email: 'sitinurmaini1@gmail.com',
    scholar: 'https://scholar.google.com/citations?user=hRzO9-8AAAAJ',
    scopus: 'https://www.scopus.com/authid/detail.uri?authorId=57191131100',
    orcid: 'https://orcid.org/0000-0002-5066-8418',
    image: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=300&h=300',
    interests: {
      en: ['Medical Robotics & Control', 'Deep Learning for Electrocardiogram', 'Medical Image Segmentation'],
      id: ['Robotika & Kontrol Medis', 'Deep Learning untuk Elektrokardiogram', 'Segmentasi Citra Medis']
    },
    currentProjects: [
      'Indonesian National Fetal Heart Screening AI Registry (Funding: BRIN)',
      'Electrocardiogram (ECG) Auto-Diagnostic & Arrhythmia Detection System'
    ],
    latestPublications: [
      'Deep Multi-Attention U-Net for Chamber Segmentation in Fetal Echocardiography Ultrasound (2025)',
      'Arrhythmia Classification on Long-term ECG Signals Using Deep Bidirectional Recurrent Networks (2024)'
    ]
  },
  {
    id: 'sukemi',
  ...
  const saveToServer = async (force: boolean = false): Promise<{ 
    ...
    try {
      ...
      try {
        ...
        if (response.status === 409) {
          ...
          return {
            success: false,
            conflict: true,
            error: result.message || 'Conflict detected! Someone else has updated the CMS on GitHub.'
          };
        }
      ...
      if (useClientFallback) {
        return {
          success: false,
          error: `Save failed: ${fallbackReason}`
        };
      }
      ...
      if (response) {
        ...
        if (result.status === 'success') {
          console.log('Saved CMS data to server successfully.');
          if (result.syncStatus && result.syncStatus.loadedSha) {
            setLoadedSha(result.syncStatus.loadedSha);
          }
          return { 
            success: true, 
            githubSync: result.githubSync
          };
        } else if (result.status === 'github_failed') {
          console.warn('Local save succeeded, but GitHub push failed:', result.error);
          return {
            success: true,
            error: result.error,
            githubSync: result.github
          };
        } else {
          return { 
            success: false, 
            error: result.error || 'Failed to save to server.' 
          };
        }
      }
